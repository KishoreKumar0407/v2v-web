          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {blogs.map((blog, index) => (
              <motion.div
                ...
                transition={{ duration: 0.5, delay: index * 0.1 }}
              >
                <Card className="h-full flex flex-col overflow-hidden rounded-[2rem] hover:shadow-xl hover:border-primary/40 transition-all duration-300 group bg-card/45 backdrop-blur-sm border-white/10">
                  <div className="relative h-52 w-full overflow-hidden bg-slate-950/40 shrink-0 border-b border-border/10">
                    {blog.image ? (
                      <img
                        src={blog.image}
                        alt={blog.title}
